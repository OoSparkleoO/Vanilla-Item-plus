...
data modify storage vanilla_item_plus:dialog dialog.body set value [{"type": "minecraft:item","item": {"id": "minecraft:command_block"},"description": {"contents": "以下是所有属于vm-3的设置项"},"show_decorations": false,"show_tooltip": false}]
data modify storage vanilla_item_plus:dialog dialog.body append value {"type": "minecraft:plain_message","contents": [{"text": "重要!!!:这些选项对游戏平衡有较强破坏性，请通过修改数据包文件开关","color":"red","bold":true},"\n",{"text":"树种转化：","color":"white"}]}
data modify storage vanilla_item_plus:dialog dialog.body[1].contents append from storage vanilla_item_plus:options_string wood_conversion

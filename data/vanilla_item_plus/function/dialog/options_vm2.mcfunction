...
data modify storage vanilla_item_plus:dialog dialog.body set value [{"type": "minecraft:item","item": {"id": "minecraft:purpur_block"},"description": {"contents": "以下是属于vm-2的选项"},"show_decorations": false,"show_tooltip": false}]
...
data modify storage vanilla_item_plus:dialog dialog.actions append value {"label": ["恢复旧版附魔金苹果配方:"]}
...
data modify storage vanilla_item_plus:dialog dialog.actions append value {"label": ["装饰用刷怪笼:"]}
...
data modify storage vanilla_item_plus:dialog dialog.actions append value {"label": ["无限宝库结构:"]}

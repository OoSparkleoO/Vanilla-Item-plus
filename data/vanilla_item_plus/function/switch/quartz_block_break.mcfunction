...
execute if data storage vanilla_item_plus:options options{quartz_block_break:0b} run tellraw @a [{"text":"选项 [vm-2]可分解石英块 已被"},{"selector":"@s"},{"text":"设为"},{"text":"关闭","color":"red","bold":true}]
execute if data storage vanilla_item_plus:options options{quartz_block_break:1b} run tellraw @a [{"text":"选项 [vm-2]可分解石英块 已被"},{"selector":"@s"},{"text":"设为"},{"text":"开启","color":"green","bold":true}]

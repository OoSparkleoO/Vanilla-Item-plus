...
execute if data storage vanilla_item_plus:options options{get_useless_spawner:0b} run tellraw @a [{"text":"选项 [vm-2]装饰性刷怪笼 已被"},{"selector":"@s"},{"text":"设为"},{"text":"关闭","color":"red","bold":true}]
execute if data storage vanilla_item_plus:options options{get_useless_spawner:1b} run tellraw @a [{"text":"选项 [vm-2]装饰性刷怪笼 已被"},{"selector":"@s"},{"text":"设为"},{"text":"开启","color":"green","bold":true}]

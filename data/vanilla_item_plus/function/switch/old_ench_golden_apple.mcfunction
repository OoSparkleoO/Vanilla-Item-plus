...
execute if data storage vanilla_item_plus:options options{old_ench_golden_apple:0b} run tellraw @a [{"text":"选项 [vm-2]恢复旧版附魔金苹果合成配方 已被"},{"selector":"@s"},{"text":"设为"},{"text":"关闭","color":"red","bold":true}]
execute if data storage vanilla_item_plus:options options{old_ench_golden_apple:1b} run tellraw @a [{"text":"选项 [vm-2]恢复旧版附魔金苹果合成配方 已被"},{"selector":"@s"},{"text":"设为"},{"text":"开启","color":"green","bold":true}]
